...
class ProductionsController extends AppController
{
    ...
    protected function setCustomFormOptions()
    {
        ...
        $this->renderOptions['products_panel'] = $this->genPanel($pns, [
            ...
            'toolbars' => [
                $this->genFilterbar($pns, 'productions_form')
            ],
            'table' => $this->genTable($pns, 'production', [
                // 'actions' => true,
                'export' => true,
                'd' => [
                    'ajax' => [
        ...
        $this->addModal([
            'title' => $this->genTranslateText('to_production', 'modal.title', $ons),
            'name' => 'orders_import',
            'en' => $ons['name'],
            'ecn' => $ons['className'],
            'fieldtype' => 'ajson',
            ...
            'toolbars' => [
                $this->genFilterbar($ons, 'productions_form')
            ],
            'buttons' => [
                'add' => [
                    'block' => 'btn_add',
                    'label' => $this->trans($this->labelText('btn.addOrders')),
                    'attr' => [
                        'class' => 'btn-success',
                        'id' => 'add_orders_btn'
                    ]
            ...
            'table' => $this->genTable($ons, 'to_production', [
                'd' => [
                    'ajax' => [
